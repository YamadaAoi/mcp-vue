...
export async function testVueSimple(): Promise<boolean> {
  const client = new MCPServerTestClient()
  const testFile = resolve('src/tests/fixtures/test-vue-simple.vue')

  console.log('=== Vue Simple Test (no Options API) ===\n')
  ...
  try {
    ...
    await client.close()
    console.error('Error testing Vue Simple:', error)
    return false
  }
}
