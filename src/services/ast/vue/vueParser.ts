...
import { extractWatchEffect } from './extractors/watchEffectExtractor'
import {
  extractProvide,
  extractInject
    ...
    if (ast) {
      ...
      if (isCompositionAPI) {
        ...
        if (watchEffects.length > 0) {
          compositionData.watchEffects = watchEffects
        }
        if (expose.length > 0) {
          compositionData.expose = expose
        }
        if (provides.length > 0) {
          compositionData.provide = provides
        }
        if (injects.length > 0) {
          compositionData.inject = injects
        }
